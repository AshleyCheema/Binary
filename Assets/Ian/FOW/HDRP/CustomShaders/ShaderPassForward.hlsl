          ...
          )
{
	...
	float3 worldSpacePostiion = GetAbsolutePositionWS(input.positionRWS);
	float3 worldSpacePostiionCenter = float4(0, 0, -2.0, 1);
	
	float fow = distance(worldSpacePostiion, worldSpacePostiionCenter);
	fow = 1.0 - smoothstep(0.0, 5.0, fow);

	//if (worldSpacePostiion.y > 0.01)
	//{
	//	outColor = float4(0, 0, 0, 1);
	//}
	//else
	//{
	//	outColor = fow == 0 ? float4(0, 0, 0, outColor.a) : outColor * fow;
	//}
	outColor = fow == 0 ? float4(0, 0, 0, outColor.a) : outColor * fow;
